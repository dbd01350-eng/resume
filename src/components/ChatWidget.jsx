...
// Smart Client-side AI Responder for static/online deployment without manual backend server
function generatePortfolioResponse(userText) {
  ...
  if (text.includes('안녕') || text.includes('hello') || text.includes('hi') || text.includes('반가')) {
    return `안녕하세요! 디자이너 겸 개발자 심다은(Daeun Sim)의 포트폴리오 AI 챗봇입니다. 🌸\n궁금한 프로젝트, 기술 스택, 경력, 연락처 정보를 물어보세요!`;
  }

  if (text.includes('연락') || text.includes('이메일') || text.includes('전화') || text.includes('문의') || text.includes('contact') || text.includes('email')) {
  ...
  if (text.includes('기술') || text.includes('스택') || text.includes('툴') || text.includes('skill') || text.includes('tool') || text.includes('언어')) {
    return `💻 사용 가능한 주요 기술 & 툴:\n- 개발: ${portfolioData.hero.tags.slice(0, 6).join(', ')}\n- 디자인: Figma, Illustrator, Premiere Pro, After Effects\n- AI 툴: Claude, GPT, AGY, Flow, Suno 등\n\n디자인과 코딩을 모두 소화하며 "예쁘게, 될 때 까지" 완성도 높은 결과를 만듭니다.`;
  }

  if (text.includes('프로젝트') || text.includes('포트폴리오') || text.includes('작품') || text.includes('project') || text.includes('ikea')) {
    const list = portfolioData.projects.map((p, i) => `${i + 1}. [${p.type}] ${p.title}`).join('\n');
    return `🚀 주요 프로젝트 목록:\n${list}\n\n사이트 내 Projects 섹션에서 상세 내용과 기획안/영상 링크를 확인하실 수 있습니다.`;
  }

  if (text.includes('경력') || text.includes('이력') || text.includes('학교') || text.includes('전공') || text.includes('학력') || text.includes('자기소개')) {
    const exp = portfolioData.experience.map(e => `• ${e.period}: ${e.title}`).join('\n');
    return `👩‍💻 심다은 님의 주요 학력 및 경력:\n${exp}\n\n다양한 공간 운영 및 설계 경험을 바탕으로 공간과 웹 사용자 경험을 입체적으로 이해합니다.`;
  }
  ...
  return `질문해 주셔서 감사합니다! 심다은 님의 프로젝트, 기술 스택(React, Figma, AI 툴), 경력, 또는 연락처(dbd01350@gmail.com)에 대해 궁금하신 점을 말씀해 주세요. 😊`;
}
...
export default function ChatWidget() {
  ...
  const send = async () => {
    ...
    // Attempt to call API server with short timeout
    try {
      ...
      // Backend not running or timeout -> Fallback seamlessly to Portfolio AI Engine
    }

    if (!botReply) {
      // Instant intelligent fallback response for static online deployments
